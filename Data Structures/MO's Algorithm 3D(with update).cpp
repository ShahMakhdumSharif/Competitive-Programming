...
void solve(ll tc)
{
    ...
    vector<pair<ll, pair<ll, ll>>> Operations;//type=1: Update, type=2:Query;
    for(ll i=0;i<q;i++)
    {
        ll t;
        cin>>t;
        if(t==1)
        {
            ...
            cin>>pos>>x;
            pos--;
            Operations.push_back({t, {pos, x}});
        }
        else
        {
            ll l, r;
            l--, r--;
            Operations.push_back({t, {l, r}});
        }
    }
    ...
    for(ll i=0;i<n;i++)
        a[i]=compress(a[i]);
    for(auto &[it, val]: Operations)
    {
        if(it==1)
            val.second=compress(val.second);
    }
    ll time=0, cnt=0;
    vector<Update> updates;
    vector<Query> queries;
    vector<ll> tem=a;
    for(auto &[type, pr]: Operations)
    {
        auto &[u, v]=pr;
        if(type==1)
        {
        ...
        {
            queries.push_back({u, v, time, cnt++});
        }
    }
    ll Block=cbrt(1LL*n*n)+1;
    ...
    vector<ll> freq(uni.size()+10, 0), cur=a, base=a;
    ll distinct=0;
    auto add=[&](ll pos)
    {
        ll x=cur[pos];
